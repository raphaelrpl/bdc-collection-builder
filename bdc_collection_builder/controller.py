...
class RadcorBusiness:
    ...
    @classmethod
    def create_synchronizer(cls, collection_id: int, name: str, start_from=None, **kwargs):
        """Create a new synchronizer in BDC cluster and set it to be listened by worker beat."""
        ...
        collection = Collection.query().filter(Collection.id == collection_id).first_or_404('Collection not found')
        model = DataSynchronizer(collection=collection, start_from=start_from, name=name, **kwargs)
        model.save()

        return dict(ok=True, name=name)

    @classmethod
    def list_synchronizers(cls, **kwargs):
        """List the available Data Synchronizer in cluster."""
